...
class learn:
    def __init__(self, actions, centers):
        ...
        self.landa = 1
        self.gama = 0.9
        self.weights = np.zeros((len(actions),len(centers)))
    ...
    def update(self):
        ...
        Zlast = np.zeros(len(self.centers)).reshape(len(self.centers), 1)
        c = 0
        self.env.render()
        ...
        while state[0] < 0.5:
            ...
            theta = theta.reshape(len(self.centers), 1)
            ...
            fi = self.generate_fi(state).reshape(len(self.centers), 1)
            Znew = fi + self.gama * self.landa * Zlast
            theta = np.add(theta, self.alpha * (1  ) * delta * Znew)
            self.weights[self.actions.index(chosen_action)] = theta.reshape(len(self.centers))
