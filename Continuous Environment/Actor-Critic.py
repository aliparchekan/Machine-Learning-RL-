...
class learn:
    def __init__(self, actions, centers, centers_action):
        ...
        self.count = [0] * len(actions)
        self.alpha_theta = 2**(-14)
        self.alpha_w = 2**(-6)
        ...
        self.avg = 0
        self.beta = 1
        self.landa_theta = 0.9
        self.landa_weights = 0.9
        self.count = 0
    ...
    def update(self):
        ...
        gamma_generator = 1
        Zlast_theta = np.zeros(len(self.centers_action)).reshape(len(self.centers_action), 1)
        Zlast_weights = np.zeros(len(self.centers)).reshape(len(self.centers), 1)
        ...
        while state[0] < 0.5:
            ...
            delta = reward + self.gama * next_value - state_value - self.avg
            self.avg = self.avg + self.beta * delta / self.count
            fi = self.generate_fi_s(state).reshape(len(self.centers), 1)

            gradient = self.gradient_of_policy(state, chosen_action)
            Zlast_weights = self.landa_weights * Zlast_weights + fi
            Zlast_theta =self.landa_theta * Zlast_theta + gradient
            self.weights_w = self.weights_w + self.alpha_w * delta * Zlast_weights
            self.theta = self.theta + self.alpha_theta * delta * Zlast_theta
