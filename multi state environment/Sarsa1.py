...
class SARSA:
    ...
    def update_Q(self, current_state, action, reward, next_state, next_action, flag):
        oldvalue = self.getQs(current_state, action)
        count = self.get_count(current_state, action)
        if flag == 0:
            next_value = self.getQs(next_state, next_action)
        else:
            next_value = 0
        if count == 0:
    ...
    def repeat_episode(self, mapbuilder):
        ...
        while current_state != mapbuilder.terminal_state():
            ...
            self.update_Q(current_state, current_action, mapbuilder.get_Reward(current_state, next_state)[1], next_state, next_action, flag)
